...
void loop() {
  ...
  if (stepper_motor_activated && (motor_direction == 1) && ((filament_ended == 1) || (temperature_read < set_temperature * 0.92))){
    stepper_motor_activated = false;
  }

  //достигла ли температура необходимого значения (минимум 92% от необходимого)
  temperature_riched = (temperature_read >= set_temperature * 0.92);
